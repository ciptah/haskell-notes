...
instance (Valid w, Eq w) => Valid (SigmaAlgebra w) where
  isValid f = 
    (forAllSets f $ \s -> s ⊆ o && f `canMeasure` (o `minus` s))
    && (forAll (countableUnions $ measurable f) $ \u -> f `canMeasure` u)
    where o = sampleSpace f

-- Constructor that checks the set of sets against w
sigmaAlgebra :: (Valid w, Eq w) =>
  Set w -> Set (Set w) -> Maybe (SigmaAlgebra w)
sigmaAlgebra o s
  | (o == sampleSpace candidate) && isValid candidate = Just candidate
...
grow2 :: (Valid w, Eq w) => Set w -> Collection (Set w) -> Collection (Set w)
...
generate :: (Valid w, Eq w) => Set w -> (Set w -> Bool) -> SigmaAlgebra w
