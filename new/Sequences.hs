...
applyOp
  :: (RealNum -> RealNum -> RealNum) -> ExtR -> ExtR -> ExtR -> ExtR -> ExtR
...
stripe_
  :: (Zero dat, Defined set (Vector n dat), Defined AllOf dat, KnownNat n)
  => Proxy n
  -> Sequence set (Vector n dat)
  -> [Sequence AllOf dat]
stripe_ proxy vseq =
...
realConverges seq limit =
  let naturals = Everything :: Naturals
  in  forAll (Everything :: Positive RealNum) $ \test ->
        thereExists naturals $ \bigN ->
          forAll (naturals % \n -> n > bigN) $ \n -> satisfy limit n test
  -- $1: type of convergence, $2: test sequence index, $3: test value
 where
  satisfy (Finite x) n test = (abs $ (seq ⬅ n) - x) < test
...
converges_
  ...
  -> ExtRD n
  -> Bool
converges_ d seq limit =
  let dim = natVal d
  in  and $ [ con i seq (limit @@ i) | i <- [0 .. (dim - 1)] ]
  where con i seq lim = realConverges (pickFn i <. seq) lim

converges
  :: (KnownNat n, Defined set (RD n)) => Sequence set (RD n) -> ExtRD n -> Bool
...
seqDim_
  :: Defined set ExtR => String -> Sequence set ExtR -> Sequence AllOf ExtR
...
limSup
  :: (Defined set (ExtRD n), KnownNat n)
  => Sequence set (ExtRD n)
  -> Vector n ExtR
...
lim
  ...
  -> Maybe (ExtRD n)
lim vseq =
  if (limSup vseq) == (limInf vseq) then Just (limSup vseq) else Nothing
...
seqa `subseq` seqb =
  thereExists (Everything :: Increasing (Sequence Positive Integer)) $ \map ->
...
convergentSubseq
  :: (KnownNat n, Defined set (RD n))
  => Sequence set (RD n)
  -> Subset (Sequence set (RD n))
convergentSubseq seq =
  everything % \s -> s `subseq` seq && (isJust $ lim $ extendFn <. s)

-- Every bounded sequence has a convergent subsequence.
bolzanoWeierstrassClaim =
  forAll
  -- Sequence of real numbers with bounded range
         ((Everything :: AllOf (Sequence AllOf R1)) % (bounded . range))
    $ \bseq -> convergentSubseq bseq /= empty

-------------- Folding Sets ------------------

-- Fold a countable infinite sequence.
foldOrdered
  :: (Defined dom a, Defined AllOf b, Eq b)
  => (a -> b -> b)
  -> b
  -> Sequence dom a
  -> Maybe (Sequence AllOf b)
foldOrdered fn zero sequence = box $ \n ->
  let candidateb = foldr fn zero $ map (f sequence) [1 .. n]
  in  if valid candidateb then candidateb else error "Invalid"
...
foldFinite
  :: (Eq a, Eq b, Defined set a, Defined AllOf b)
  => (a -> b -> b)
  -> b
  -> set a
  -> Subset b
foldFinite fn zero set | finite set =
  everything % \x -> thereExists (toList set) $ \lst -> foldr fn zero lst == x
...
foldCountable
  ...
  -> b
  -> dom a
  -> Subset (Sequence AllOf b)
foldCountable fn zero set | countable set =
  collapse $ smap (foldOrdered fn zero) $ mappers
    (Everything :: Positive Integer)
    set
...
series seq = mustHave "summation of fine Num terms must succeed" $ box $ \n ->
  sum $ map (f seq) [1 .. n]
...
sumSeq
  :: (KnownNat n, Num (ExtRD n), Defined cod (ExtRD n))
  => Sequence cod (ExtRD n)
  -> ExtRD n
sumSeq seq = mustHave "see comment above" $ lim $ series seq

unionSeq
  :: (Eq (set w), Defined set w, Defined cod (set w))
  => Sequence cod (set w)
  -> Subset w
